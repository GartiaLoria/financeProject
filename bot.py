...
# from telegram import Update
# from telegram.ext import ApplicationBuilder, ContextTypes, MessageHandler, filters
# from utils import parse_expense_with_gemini, add_expense, get_chat_response, collection

# TELEGRAM_TOKEN = "8276467916:AAEOOj2hYO1kOsG1Zl_FmECrjJcaRxj-tEA"
# DASHBOARD_URL = "http://localhost:8501" # Or your deployed Streamlit URL

# async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
#     user_text = update.message.text
#     user_id = update.effective_user.id
    
#     # 1. Check if it's a question (simple heuristic)
#     if "?" in user_text or "how much" in user_text.lower() or "show" in user_text.lower():
#         # Fetch recent data for context (limit to last 50 for context window efficiency)
#         cursor = collection.find({}, {"_id": 0}).sort("date", -1).limit(50)
#         data_context = list(cursor)
        
#         answer = get_chat_response(user_text, str(data_context))
#         await update.message.reply_text(answer)
        
#     # 2. Otherwise, treat it as an expense entry
#     else:
#         processing_msg = await update.message.reply_text("⏳ Parsing expense...")
        
#         parsed_data = parse_expense_with_gemini(user_text)
        
#         if parsed_data:
#             add_expense(parsed_data)
#             response_msg = (
#                 f"✅ **Saved!**\n"
#                 f"Item: {parsed_data['i']}\n"
#                 f"Amount: {parsed_data['a']}\n"
#                 f"Category: {parsed_data['a']}\n\n"
#                 f"View details: {DASHBOARD_URL}"
#             )
#             await context.bot.edit_message_text(chat_id=user_id, message_id=processing_msg.message_id, text=response_msg, parse_mode='Markdown')
#         else:
#             await context.bot.edit_message_text(chat_id=user_id, message_id=processing_msg.message_id, text="❌ Couldn't understand that. Try: '20$ for taxi'")
